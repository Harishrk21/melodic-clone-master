...
import { ArrowRight, Users, MapPin, Sparkles, Crown, Music, Wrench, Guitar, Piano } from "lucide-react";
...
const Hero = () => {
  ...
  return (
    <section className="relative overflow-hidden bg-black py-24 md:py-36">
      ...
      {/* Instrument Service Section */}
      <div className="container mx-auto px-4 relative z-10 mt-20">
        <div className="relative overflow-hidden bg-gradient-to-br from-amber-950/80 via-black to-yellow-950/60 rounded-3xl border-2 border-amber-500/40 backdrop-blur-xl shadow-2xl shadow-amber-500/30">
          {/* Animated background elements */}
          <div className="absolute inset-0 bg-[linear-gradient(45deg,transparent_25%,rgba(217,175,55,0.05)_50%,transparent_75%)] bg-[length:250%_250%] animate-shimmer" />
          ...
          <div className="relative z-10 p-8 md:p-12 lg:p-16">
            {/* Section Badge */}
            <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-blue-900/40 to-blue-800/40 border border-blue-400/40 backdrop-blur-sm shadow-lg shadow-blue-500/20 mb-8">
              <Wrench className="h-4 w-4 text-blue-400" />
              <span className="text-sm font-semibold bg-gradient-to-r from-blue-300 to-blue-400 bg-clip-text text-transparent">
                Professional Instrument Care
              </span>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    <div className="h-14 w-14 rounded-xl bg-gradient-to-br from-amber-500 via-yellow-500 to-amber-600 flex items-center justify-center shadow-lg shadow-amber-500/50">
                      <Guitar className="h-7 w-7 text-black" />
                    </div>
                ...
                <p className="text-lg text-gray-300 leading-relaxed">
                  Professional servicing and repair for all musical instruments. Our certified technicians provide expert care with precision and attention to detail.
                </p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div 
                      key={index}
                      className="group flex items-center gap-2 bg-gradient-to-r from-amber-950/40 to-yellow-950/30 p-3 rounded-xl border border-amber-500/20 hover:border-amber-400/50 transition-all duration-300 hover:shadow-lg hover:shadow-amber-500/20"
                    >
                      <div className="h-2 w-2 rounded-full bg-gradient-to-r from-amber-400 to-yellow-400 group-hover:animate-pulse" />
                      <span className="text-sm font-medium text-amber-200/90 group-hover:text-amber-100 transition-colors">{service}</span>
                    </div>
                  ))}
                </div>
                ...
                <Button 
                  asChild 
                  size="lg" 
                  className="group relative bg-gradient-to-r from-blue-600 via-blue-500 to-blue-600 hover:from-blue-500 hover:via-blue-400 hover:to-blue-500 text-white font-bold px-8 py-6 text-lg rounded-xl shadow-2xl shadow-blue-500/50 hover:shadow-blue-400/70 transition-all duration-300 border-0"
                >
                  <a href="/contact" className="relative flex items-center gap-2 z-10">
                    <Wrench className="h-5 w-5" />
                    Book Instrument Service
                    <ArrowRight className="h-5 w-5 group-hover:translate-x-1 transition-transform" />
                  </a>
                </Button>
              </div>
              ...
              <div className="relative">
                {/* Glowing background */}
                <div className="absolute inset-0 bg-gradient-to-br from-amber-500/20 to-yellow-500/20 rounded-2xl blur-2xl" />
                
                <div className="relative grid grid-cols-2 gap-4">
                  {[
                    { name: "Guitar", icon: "🎸", color: "from-amber-500 to-yellow-500" },
                    { name: "Keyboard", icon: "🎹", color: "from-blue-500 to-cyan-500" },
                    { name: "Drums", icon: "🥁", color: "from-red-500 to-orange-500" },
                    { name: "Microphone", icon: "🎤", color: "from-purple-500 to-pink-500" },
                    { name: "Flute", icon: "🪈", color: "from-green-500 to-emerald-500" },
                    { name: "Violin", icon: "🎻", color: "from-amber-600 to-yellow-600" },
                    { name: "Chenda Melam", icon: "🪘", color: "from-orange-500 to-red-500" },
                    { name: "All Instruments", icon: "🎼", color: "from-amber-400 to-yellow-400" }
                  ].map((instrument, index) => (
                    <div 
                      key={index}
                      className="group relative bg-gradient-to-br from-black/80 to-amber-950/60 p-6 rounded-2xl border border-amber-500/30 hover:border-amber-400/60 backdrop-blur-sm transition-all duration-500 hover:shadow-xl hover:shadow-amber-500/30 hover:scale-105"
                    >
                      <div className="absolute inset-0 bg-gradient-to-br from-amber-500/0 to-amber-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                      
                      <div className="relative flex flex-col items-center gap-3 text-center">
                        <div className={`h-16 w-16 rounded-xl bg-gradient-to-br ${instrument.color} flex items-center justify-center shadow-lg text-3xl group-hover:shadow-2xl transition-all duration-300 group-hover:scale-110`}>
                          {instrument.icon}
                        </div>
                        <span className="text-sm font-semibold text-amber-200/90 group-hover:text-amber-100 transition-colors">
                          {instrument.name}
                        </span>
                      </div>
                      
                      {/* Decorative corner accents */}
                      <div className="absolute top-2 right-2 w-3 h-3 border-t-2 border-r-2 border-amber-400/40 group-hover:border-amber-300/60 transition-colors" />
                      <div className="absolute bottom-2 left-2 w-3 h-3 border-b-2 border-l-2 border-amber-400/40 group-hover:border-amber-300/60 transition-colors" />
                    </div>
                  ))}
                </div>

                {/* Floating particles */}
                <div className="absolute -top-4 -right-4 w-3 h-3 bg-blue-400 rounded-full animate-ping opacity-75" />
                <div className="absolute top-1/2 -left-4 w-2 h-2 bg-amber-400 rounded-full animate-ping opacity-75" style={{ animationDelay: '0.5s' }} />
                <div className="absolute -bottom-4 -right-4 w-4 h-4 bg-yellow-400 rounded-full animate-ping opacity-75" style={{ animationDelay: '1s' }} />
              </div>
            </div>
          </div>
        </div>
